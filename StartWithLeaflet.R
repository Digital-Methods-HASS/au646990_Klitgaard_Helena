...
#Answer 1: first longitude, then latitude
...
#Answer 2: with T the layer-control-box is able to collapse (T=TRUE)
# With F the layer-control-box is open constantly (F=False)
...
leaflet() %>% 
  setView(9.331402, 55.974925, zoom = 8) %>%
  addTiles()  # checking I am in the right area
...
DANmap %>%
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             popup = paste(places$Description, "<br>", places$Type),clusterOptions = markerClusterOptions(freezeAtZoom = 6))
...
# Your brief answer
#The Cluster option helps to identify where you can find the biggest number of places
#The unclustered option gives you a batter overview of the precise locations
...
DANmap %>%
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             popup = paste(places$Placename, "<br>",places$Description, "<br>", places$Type),clusterOptions = markerClusterOptions(freezeAtZoom = 6))
